    ...
    <!-- fin modal-->
     ...
     $(document).ready(function(){  
          ...
          $('#add_button').click(function(){  
               $('#user_form')[0].reset();  
               $('.modal-title').text("Paramètrage des postes");  
               $('#action').val("Add");  
          })  
          ...
          var dataTable = $('#user_data').DataTable({  
               ...
               "order":[],  
               "ajax":{  
                    url:"<?php echo base_url() . 'admin/fetch_poste'; ?>",  
                    type:"POST"  
               },  
               "columnDefs":[  
                    {  
               ...
               if(designation != '')
                {
                  ...
                  if (action =="Add") {
                      ...
                      $.ajax({  
                           url:"<?php echo base_url() . 'admin/operation_poste'?>",  
                           method:'POST',  
                  ...
                  if (action == 'Edit') {
                        ...
                        $.ajax({  
                             url:"<?php echo base_url() . 'admin/modification_poste'?>",  
                             method:'POST',  
                             data:new FormData(this),  
                             contentType:false,  
                             processData:false,  
          ...
          $(document).on('click', '.update', function(){  
               var idposte = $(this).attr("idposte");  
               $.ajax({  
                    url:"<?php echo base_url(); ?>admin/fetch_single_poste",  
                    method:"POST",  
                    ...
                    {  
                         ...
                         $('.modal-title').text("modification de poste "+data.designation);  
                         $('#idposte').val(idposte);   
                         $('#action').val("Edit");  
                    }  
               });  
              ...
              if(confirm("Are you sure you want to delete this?"))
		          {
		           		...
		           		$.ajax({
	                    url:"<?php echo base_url(); ?>admin/supression_poste",
	                    method:"POST",
	                    data:{idposte:idposte},
	                    success:function(data)
	                    {
